...
-- TCP Transport
-- 
-- script-name: twcssp_server_mbo.lua
-- Version 0.3
-- 
...
ssp_fields.pf_desc_video_decode_type        = ProtoField.new("Video Decode Type", "dsmcc_ssp_udp.desc_video_decode_type", ftypes.BYTES) -- uint16("dsmcc_ssp_tcp.desc_video_decode_type", "Video Decode Type", base.HEX, video_decode_type)
...
function twcssp.dissector(tvbuf,pktinfo,root)
    ...
    if messageid == 0x8012 then
        ...
        if (protocol_id == 2) then
            ...
            for j=desc_count,1,-1 do
                ...
                elseif (desc_tag == 196) then
                    ...
                    local desc_length = tvbuf:range(pos,1):uint()
                    pos = pos + 1
                    desc_tree:add(ssp_fields.pf_desc_video_decode_type, tvbuf:range(pos,desc_length))
                    pos = pos + desc_length
        else
        ...
        -- SSP version 1 ?
            ...
            tree:add("Unknown Protocol ID, Check Mystro specs!")
        end
    ...
    elseif messageid == 0x8020 then
        ...
        local session = tvbuf:range(pos,4):uint()
        pos = pos + 4
        pktinfo.cols.info = "Server Release Request (SRRQ), mac="..mac .. ", session="..session
    ...
    ------------------------------
    --  Server Release Confirm  --
    ------------------------------
    elseif messageid == 0x8021 then
        local folder_tree = tree:add("Session")
        pktinfo.cols.info = "Server Release Confirm (SRC)"
    ...
    elseif messageid == 0x8022 then
        ...
        local session = tvbuf:range(pos,4):uint()        
        pos = pos + 4
        pktinfo.cols.info = "Server Release Indication (SRI), mac="..mac .. ", session="..session
        local folder_tree = tree:add("Reason Code")
        folder_tree:add(ssp_fields.pf_reason, tvbuf:range(pos,2))
        pos = pos + 2
        local folder_tree = tree:add("User Private Data")
    ...
    elseif messageid == 0x8023 then
        ...
        local session = tvbuf:range(pos,4):uint()        
        pos = pos + 4
        pktinfo.cols.info = "Server Release Response (SRR), mac="..mac .. ", session="..session
        local folder_tree = tree:add("User Private Data")
        folder_tree:add(ssp_fields.pf_uudata_length, tvbuf:range(pos,2))
        pos = pos + 2
        folder_tree:add(ssp_fields.pf_private_data_length, tvbuf:range(pos,2))
            ...
            for i=resource_count,1,-1 do
                ...
                if (resource_type == 3) or (resource_type == 6) or (resource_type == 61444) or (resource_type == 61446) or (resource_type == 61447) then
                    ...
                    if (resource_type == 3) then  -- mpeg program
                        ...
                        pos = pos + 2
            -- extra bytes
            resource_tree:add(ssp_fields.pf_unknown, tvbuf:range(pos,2))
                        pos = pos + 2
    ...
    elseif messageid == 0x8061 then
        ...
        local session = tvbuf:range(pos,4):uint()        
        pos = pos + 4
        pktinfo.cols.info = "Server Status Confirm (SSC), mac="..mac .. ", session="..session
        bytes_remaining = pktlen - pos
        if bytes_remaining > 0 then
            folder_tree:add(ssp_fields.pf_unknown, tvbuf:range(pos,bytes_remaining))
        end
